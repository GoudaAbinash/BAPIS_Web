...
@Component({
  selector: "app-fireform",
  templateUrl: "./fireform.component.html",
  styleUrls: ["./fireform.component.scss"],
})
export class FireFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.quoteGeneraion = new FormGroup({
      ...
      coverType: new FormControl("LTD", Validators.required),
  ...
  generatePDF(ucoPayment) {
    ...
    console.log(obj);
    this.api.generateFireLongPDF(obj).subscribe((sus) => {
      console.log(sus);
      this.loading = false;
      if (sus.ResponseFlag == 1) {
        let res = JSON.parse(sus["ResponseMessage"]).Table;
