...
export class FSLComponent implements OnInit {
  ...
  distancewaterbodycode = "";
  Watchmanondutycode = "";
  goodhousecode="";
  BasementStorcode="";
  Stockcompositioncode = "";
  manufacturingunitcode = "";
  FloorsofBuildingcode = "";
  Constructioncode = "";
  electInswiringcode="";
  Averagequipmentcode = "";
  ...
  ngOnInit(): void {
    ...
    this.quoteGeneraion = new FormGroup({
      ...
      coverType: new FormControl("FSL", Validators.required),
      ...
      buildingAge: new FormControl('', null),
      constructDetails: new FormControl("", null),
      distancebrigad: new FormControl("", null),
      ageOfBuildingCode: new FormControl("", null),
      distancewaterbody: new FormControl("", null),
      Watchmanonduty: new FormControl("", null),
      GoodHouse: new FormControl("5", null),
      BasementStorage: new FormControl("", null),
      Stockcomposition: new FormControl("", null),
      manufacturingunit: new FormControl("", null),
      FloorsofBuilding: new FormControl("", null),
      Construction: new FormControl("", null),
      electInswiring: new FormControl("5", null),
      Averagequipment: new FormControl("", null),
 ...
 }else
 {
  ...
  createClient(receiptData, ucoPayment) {
    ...
    this.api.createClient(obj).subscribe((sus) => {
      ...
      if (sus.ResponseFlag == 1) {
        //JSON.parse(data.ResponseMessage).Client[0].Status
  ...
  generatePDF(ucoPayment) {
    ...
    obj.distancewaterbody = this.distancewaterbodycode;
    obj.Watchmanonduty = this.Watchmanondutycode;
    obj.goodhousecode = this.goodhousecode;
    obj.BasementStorcode = this.BasementStorcode;
    obj.Stockcomposition = this.Stockcompositioncode;
    obj.manufacturingunit = this.manufacturingunitcode;
    obj.FloorsofBuilding = this.FloorsofBuildingcode;
    obj.Construction = this.Constructioncode;
    obj.electInswiring = this.electInswiringcode;
    obj.Averagequipment = this.Averagequipmentcode;
    obj.payerID = this.payerid;
    obj.receiptvendorname = this.receiptvendorname;
    obj.burgalarypolicyNo = this.burgalarypolicyNo;
     obj.commercialod = this.commercialod;
    ...
    console.log(obj);
    this.api.generateFSLPolicy(obj).subscribe((sus) => {
      console.log(sus);
      this.loading = false;
      if (sus.ResponseFlag == 1) {
        let res = JSON.parse(sus["ResponseMessage"]).Table;
  ...
  setRateAsPerOccupation(val) {
    ...
    if (o) {
      ...
      this.basicOoccupationRateTemp = o.Zone[this.currentZoneNo - 1];
  ...
  setDiscount() {
    let commercialod=this.quoteGeneraion.value.commercialoding < 0 ? 0: this.quoteGeneraion.value.commercialoding==null ? 0:this.quoteGeneraion.value.commercialoding;
    console.log(this.quoteGeneraion.value);
    ...
    let distancebrigad = this.quoteGeneraion.value.distancebrigad;
    let distancewaterbody = this.quoteGeneraion.value.distancewaterbody;
    let Watchmanonduty = this.quoteGeneraion.value.Watchmanonduty;
    let goodhouse = this.quoteGeneraion.value.GoodHouse;
    let basementstorage = this.quoteGeneraion.value.BasementStorage;
    let Stockcomposition = this.quoteGeneraion.value.Stockcomposition;
    let manufacturingunit = this.quoteGeneraion.value.manufacturingunit;
    let FloorsofBuilding = this.quoteGeneraion.value.FloorsofBuilding;
    let Construction = this.quoteGeneraion.value.Construction;
    let electInswiring = this.quoteGeneraion.value.electInswiring;
    let Averagequipment = this.quoteGeneraion.value.Averagequipment;

    this.ageOfBuildingCode = ageBuild == "0" ? "0010" : ageBuild == "5" ? "0015" : ageBuild == "10" ? "0099" : "NOTA";
    this.fireProtectionCode = fireProtet == "5" ? "NFEA" : "NOTA";
    this.constructDetailsCode = constructDetails == "0" ? "PUKA" : constructDetails == "5" ? "KUCH" : "NOTA";
    this.distancebrigad = distancebrigad == "0" ? "0020" : distancebrigad == "5" ? "0099" : "NOTA";
    this.distancewaterbodycode = distancewaterbody == "0" ? "0250" : distancewaterbody == "10" ? "0249" : "NOTA";
    this.Watchmanondutycode = Watchmanonduty == "5" ? "NONO" : "NOTA";
    this.BasementStorcode=basementstorage=="5"?"YESS":basementstorage=="0"?"NONO":"NOTA";
    ...
    Number(commercialod)+
      ...
      Number(distancewaterbody) +
      Number(Watchmanonduty) +
      Number(goodhouse) +
      Number(basementstorage) +
      Number(Stockcomposition) +
      Number(manufacturingunit) +
      Number(FloorsofBuilding) +
      Number(Construction) +
      Number(electInswiring) +
      Number(Averagequipment);
